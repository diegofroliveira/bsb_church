...
// Simple similarity check for duplicate names
const similar = (a: string, b: string): boolean => {
  if (a === b) return false; // exact same after normalize = handled elsewhere
  const shorter = a.length < b.length ? a : b;
  const longer = a.length < b.length ? b : a;
  if (longer.startsWith(shorter + ' ') || shorter.startsWith(longer.split(' ').slice(0, 2).join(' '))) return true;
  return false;
};

const severityLabel: Record<string, string> = { high: 'Alta', medium: 'MÃ©dia', low: 'Baixa' };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const semDisc = membros.filter(m => {
          ...
          if (!['membro', 'lÃ­der', 'lider', 'diÃ¡cono', 'diacono'].some(t => tipo.includes(t))) return false;
          return !discipulos.has(normalizeStr(m.nome));
        });
        newReports.push({ id: 'sem_disc', title: 'Membros sem Discipulador', description: 'Membros/LÃ­deres/DiÃ¡conos ativos que nÃ£o aparecem como discÃ­pulo de ninguÃ©m.', count: semDisc.length, severity: 'high', data: semDisc.map(m => ({ nome: m.nome, tipo: m.tipo_cadastro })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'tipo', label: 'Tipo' }] });
        ...
        const semGrupo = membros.filter(m => {
          ...
          return ['membro', 'pastor', 'lÃ­der', 'lider', 'diÃ¡cono', 'diacono'].some(t => tipo.includes(t));
        });
        newReports.push({ id: 'sem_grupo', title: 'Ativos sem Grupo Caseiro', description: 'Membros ativos que deveriam estar vinculados a uma cÃ©lula, mas o campo estÃ¡ vazio.', count: semGrupo.length, severity: 'high', data: semGrupo.map(m => ({ nome: m.nome, tipo: m.tipo_cadastro })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'tipo', label: 'Tipo' }] });
        ...
        newReports.push({ id: 'disc_inativo', title: 'Discipuladores Inativos', description: 'Pessoas inativas que ainda constam como lÃ­deres de alguÃ©m na rede de discipulado.', count: discInativos.length, severity: 'high', data: discInativos.map(nome => ({ nome, status: membrosMap.get(nome)?.status || 'Inativo' })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'status', label: 'Status' }] });
        ...
        newReports.push({ id: 'celula_sem_lider', title: 'Grupos sem LÃ­der VÃ¡lido', description: 'CÃ©lulas cujo lÃ­der estÃ¡ vazio, nÃ£o existe na base ou estÃ¡ inativo.', count: celulasSemLider.length, severity: 'high', data: celulasSemLider.map(c => ({ grupo: c.grupo_caseiro, lider: c.lider || '(vazio)', setor: c.setor })), columns: [{ key: 'grupo', label: 'Grupo' }, { key: 'lider', label: 'LÃ­der Atual' }, { key: 'setor', label: 'Setor' }] });
        ...
        newReports.push({ id: 'loop_rede', title: 'Loops no Discipulado', description: 'Discipulado circular (A â†’ B â†’ A), que quebra hierarquias lÃ³gicas.', count: cycles.length, severity: 'high', data: cycles.map((c, i) => ({ loop: c.join(' âž” '), num: i + 1 })), columns: [{ key: 'num', label: '#' }, { key: 'loop', label: 'Ciclo Detectado' }] });
        ...
        newReports.push({ id: 'grupo_fantasma', title: 'Membros em Grupos Inexistentes', description: 'Membros associados a um Grupo Caseiro que nÃ£o existe na tabela oficial de CÃ©lulas.', count: grupoFantasma.length, severity: 'medium', data: grupoFantasma.map(m => ({ nome: m.nome, grupo: m.grupos_caseiros })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'grupo', label: 'Grupo Digitado' }] });

        // 7. Nomes Ã“rfÃ£os no Discipulado
        const typos = [...allDiscNames].filter(nome => nome && !membrosNomes.has(nome));
        newReports.push({ id: 'typo_disc', title: 'Nomes Ã“rfÃ£os no Discipulado', description: 'Nomes na tabela de Discipulado que nÃ£o tÃªm cadastro correspondente na tabela de Membros.', count: typos.length, severity: 'medium', data: typos.map(nome => ({ nome: discOriginalNames.get(nome) || nome })), columns: [{ key: 'nome', label: 'Nome no Discipulado' }] });
        ...
        newReports.push({ id: 'sem_tel', title: 'Membros Ativos sem Telefone', description: 'Cadastros sem celular dificultam contato e comunicaÃ§Ã£o pastoral.', count: semTel.length, severity: 'low', data: semTel.map(m => ({ nome: m.nome, tipo: m.tipo_cadastro })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'tipo', label: 'Tipo' }] });

        // 9. Ativos sem Email
        const semEmail = membros.filter(m => m.status === 'Ativo' && (!m.email || m.email.trim() === '' || !m.email.includes('@')));
        newReports.push({ id: 'sem_email', title: 'Membros Ativos sem Email', description: 'Cadastros sem e-mail vÃ¡lido limitam comunicaÃ§Ã£o digital e convites.', count: semEmail.length, severity: 'low', data: semEmail.map(m => ({ nome: m.nome, tipo: m.tipo_cadastro })), columns: [{ key: 'nome', label: 'Nome' }, { key: 'tipo', label: 'Tipo' }] });

        // 10. CÃ©lulas Acima da Capacidade
        ...
        newReports.push({ id: 'duplicatas', title: 'PossÃ­veis Membros Duplicados', description: 'Nomes muito semelhantes que podem representar o mesmo cadastro em duplicidade.', count: dupeGroups.length, severity: 'medium', data: dupeGroups.map((d, i) => ({ num: i + 1, nome_a: d.a, nome_b: d.b })), columns: [{ key: 'num', label: '#' }, { key: 'nome_a', label: 'Nome A' }, { key: 'nome_b', label: 'Nome B' }] });
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto pb-12">
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500 max-w-2xl">
            Identifique inconsistÃªncias, cadastros Ã³rfÃ£os e quebras de regras de negÃ³cio para manter a saÃºde do banco de dados.
          </p>
        </div>
        {!isLoading && (
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {(['all', 'high', 'medium', 'low'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? 'Todos os relatÃ³rios' : `Severidade ${severityLabel[f]}`}
            </button>
          ))}
        </div>
      )}
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 flex flex-col items-center justify-center min-h-[400px]">
          ...
          <p className="text-gray-400 text-sm mt-1">Cruzando membros, cÃ©lulas e vÃ­nculos de discipulado.</p>
        </div>
      ) : (
